...
data.combined@reductions$umap@cell.embeddings %>%
  ...
  write_tsv("../../out/table/analysis_R45_pixi/28_LYM_subcluster_HarmonySample_coordUMAP.tsv")

DefaultAssay(data.combined) <- "RNA"
object_diet <- DietSeurat(object = data.combined, assays = "RNA")
saveRDS(object_diet,"../../out/object/analysis_R45_pixi/28_LYM_subcluster_HarmonySample_diet.rds")
...
DimPlot(reference$plot,group.by = "celltype.l1",repel = T,label = T)

# save the reference umap
df_point <- reference$map@reductions$refUMAP@cell.embeddings %>%
  data.frame() %>%
  rownames_to_column("barcode")
write_tsv(df_point,"../../out/table/analysis_R45_pixi/azimuth_pbmcref_CoordUMAP.tsv")

# save the meta of the ref
df_meta <- reference$map@meta.data %>%
  data.frame() %>%
  rownames_to_column("barcode")
write_tsv(df_meta,"../../out/table/analysis_R45_pixi/azimuth_pbmcref_Metadata.tsv")
