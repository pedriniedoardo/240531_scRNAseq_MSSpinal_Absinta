...
# subclustering itself is unchanged from R44, only the annotation LUT changed, so these still point at the R44 objects
# for EPENDYMA and SWAHNN cells we have decided to keep them like this, also the populations are very small anyway
rds_lut <- c(
...
# load the annotation summary produced by aletta's further cleanup pass
lut <- read_csv("../../data/260816_spinal_sublcuster_annotation_aletta_v2.csv") %>%
...
list_out <- lapply(names(rds_lut), function(pop_name) {
  ...
  if (nrow(lut_pop) == 0) {
    ...
    df_annotate <- data.frame(barcode = colnames(sobj),
                              cell_id_subcluster = pop_name,
                              cell_id_subcluster2 = pop_name,
                              cell_id_subcluster3 = pop_name)
  ...
  output_plot <- paste0("../../out/plot/analysis_R45_pixi/02_", pop_name, "_subcluster_filterAnnotate_v2.pdf")
  ...
  if (nrow(lut_annotate) == 0) {
    ...
    df_annotate <- tibble(barcode = character(0),
                          cell_id_subcluster = character(0),
                          cell_id_subcluster2 = character(0),
                          cell_id_subcluster3 = character(0))
  } else {
    df_annotate <- pmap_dfr(lut_annotate, function(cluster_id, cell_id_subcluster, cell_id_subcluster2, cell_id_subcluster3, ...) {
      data.frame(
        barcode = WhichCells(sobj, idents = cluster_id),
        subcluster_id = pop_name,
        cell_id_subcluster = cell_id_subcluster,
        cell_id_subcluster2 = cell_id_subcluster2,
        cell_id_subcluster3 = cell_id_subcluster3
      )
    })
  ...
  # shortlit the table
  test_meta <- lut_annotate %>% dplyr::select(subcluster_id, col_meta, cluster_id, action, cell_id_subcluster, cell_id_subcluster2, cell_id_subcluster3)
  ...
  p01 <- DimPlot(sobj, group.by = col_meta, label = TRUE) + ggtitle(paste0(col_meta," - subcluster"))
  p02 <- DimPlot(sobj, group.by = "flag_remove", cols = c("TRUE" = "red", "FALSE" = "grey80")) + ggtitle("flagged for removal")
  p03 <- DimPlot(sobj, group.by = "cell_id_subcluster3", label = TRUE) + ggtitle("consensus annotation")
  ggsave(filename = output_plot, plot = p01 | p02 | p03, height = 6, width = 18)
...
saveRDS(list_out,"../../out/object/analysis_R45_pixi/02_barcodes_annotation_v2.rds")
