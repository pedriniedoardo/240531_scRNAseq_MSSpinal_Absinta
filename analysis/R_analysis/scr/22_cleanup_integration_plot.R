...
# read in the data --------------------------------------------------------
data.combined <- readRDS("../../out/object/manualClean/20_sobj_integrated_cleanup.rds")
...
clustree::clustree(data.combined@meta.data[,grep("RNA_snn_res", colnames(data.combined@meta.data))],
                   prefix = "RNA_snn_res.")
ggsave("../../out/plot/22_UMAPCluster_tree.pdf",width = 10,height = 10)
...
ggsave("../../out/plot/22_UMAPCluster_resolutions.pdf",width = 25,height = 15)
...
ggsave(plot=test_long01,"../../out/plot/22_DotplotLong_sobj_integrated_cleanup_res0.1.pdf",width = 30,height = 6)
...
test_long04 <- DotPlot(data.combined,
  ...
  theme(strip.text = element_text(angle = 90))
ggsave(plot=test_long04,"../../out/plot/22_DotplotLong_sobj_integrated_cleanup_res0.4.pdf",width = 30,height = 6)
...
ggsave("../../out/plot/22_UMAPCluster_ExpertAnnotaiton_long.pdf",width = 22,height = 15)
...
# ggsave("../../out/plot/22_ViolinCluster_ExpertAnnotaiton_res0.2.pdf",width = 7,height = 21)
# -------------------------------------------------------------------------
...
ggsave("../../out/plot/22_UMAPCluster_technical.pdf",width = 10,height = 8)
...
#   theme(strip.background = element_blank(),
#         axis.text.x = element_text(hjust = 1,angle = 45))
# ggsave("../../out/plot/22_ViolinCluster_technical_res0.1.pdf",width = 7,height = 12)
# -------------------------------------------------------------------------
...
ggsave(plot=plot03,"../../out/plot/22_UMAPCluster_sobj_integrated_cleanup_res0.1.pdf",width = 6,height = 5)
...
ggsave(plot=plot03a,"../../out/plot/22_UMAPClusterSplit_sobj_integrated_cleanup_res0.2.pdf",width = 13,height = 12)

# main umap cell cycle
plot04 <- DimPlot(data.combined, reduction = "umap", group.by = "Phase",raster = T,order = T)
ggsave(plot=plot04,"../../out/plot/22_UMAPPhase_sobj_integrated_cleanup.pdf",width = 5,height = 4)
...
ggsave("../../out/plot/22_BarplotPhase_summary_sobj_integrated_cleanup_res0.1.pdf",width = 7,height = 6)
...
ggsave("../../out/plot/22_BarplotPhase2_summary_sobj_integrated_cleanup_res0.1.pdf",width = 7,height = 20)
...
df_summary <- df_meta %>%
  ...
  ungroup() %>%
  mutate(prop=n/tot)
write_tsv(df_summary,"../../out/table/22_summary_sobj_integrated_cleanup_res0.1.tsv")
...
ggsave("../../out/plot/22_Barplot_summary_sobj_integrated_cleanup_res0.1.pdf",width = 7,height = 6)
...
df_summary_02 <- df_meta %>%
  ...
  ungroup() %>%
  mutate(prop=n/tot)
write_tsv(df_summary_02,"../../out/table/22_summary02_sobj_integrated_cleanup_res0.1.tsv")
...
df_summary_02 %>%
  ...
  theme(axis.text.x = element_text(hjust = 1,angle = 90))+
  scale_y_sqrt()
ggsave("../../out/plot/22_BoxplotLocation_summary_sobj_integrated_cleanup_res0.1.pdf",width = 8,height = 5)
...
df_summary_02 %>%
  ...
  theme(axis.text.x = element_text(hjust = 1,angle = 90))+
  scale_y_sqrt()
ggsave("../../out/plot/22_BoxplotDiagnosis_summary_sobj_integrated_cleanup_res0.1.pdf",width = 8,height = 5)
...
pdf("../../out/plot/22_HeatmapCluster_summary_sobj_integrated_cleanup_res0.1.pdf",width = 10,height = 6)
...
pdf("../../out/plot/22_HeatmapSample_summary_sobj_integrated_cleanup_res0.1.pdf",width = 10,height = 6)
...
pdf("../../out/plot/22_HeatmapSampleProp_summary_sobj_integrated_cleanup_res0.1.pdf",width = 10,height = 6)
...
sobj_total_h.markers %>%
  write_tsv("../../out/table/22_FindAllMarkers_sobj_integrated_cleanup_res0.1.tsv")

# save the top 100
sobj_total_h.markers %>%
  group_by(cluster) %>%
  dplyr::slice(1:100) %>%
  write_tsv("../../out/table/22_FindAllMarkers_sobj_integrated_cleanup_res0.1_top100.tsv")
...
sobj_total_h.markers2 %>%
  write_tsv("../../out/table/22_FindAllMarkers_sobj_integrated_cleanup_res0.4.tsv")

# save the top 100
sobj_total_h.markers2 %>%
  group_by(cluster) %>%
  dplyr::slice(1:100) %>%
  write_tsv("../../out/table/22_FindAllMarkers_sobj_integrated_cleanup_res0.4_top100.tsv")
...
ggsave("../../out/plot/manualClean/TopMarkersDitto_sobj_integrated_cleanup_res0.2.pdf",width = 15,height = 6)

# save the object ---------------------------------------------------------
# save the object with the full annotation
saveRDS(data.combined,"../../out/object/manualClean/data.combined_sobj_integrated_cleanup_Annotation.rds")
