...
# read in the data --------------------------------------------------------
data.combined <- readRDS("../../out/object/17_IMMUNE_subcluster_HarmonySample.rds")
...
clustree::clustree(data.combined@meta.data[,grep("RNA_snn_res", colnames(data.combined@meta.data))],
                   prefix = "RNA_snn_res.")
ggsave("../../out/plot/subclusters/18_UMAPCluster_tree_IMMUNE_subcluster.pdf",width = 10,height = 10)
...
ggsave("../../out/plot/subclusters/18_UMAPCluster_resolutions_IMMUNE_subcluster.pdf",width = 25,height = 15)

# general UMAP with new clustering
DimPlot(data.combined, reduction = "umap",group.by = "RNA_snn_res.0.1",label = T,raster = T)
ggsave("../../out/plot/subclusters/18_UMAPCluster_res0.1_IMMUNE_subcluster.pdf",width = 6,height = 5)

DimPlot(data.combined, reduction = "umap",group.by = "RNA_snn_res.0.4",label = T,raster = T)
ggsave("../../out/plot/subclusters/18_UMAPCluster_res0.4_IMMUNE_subcluster.pdf",width = 6,height = 5)
...
# ggsave("../../out/plot/subclusters/18_UMAPCluster_splitTreat_res0.1_IMMUNE_subcluster.pdf",width = 6,height = 3)
...
ggsave("../../out/plot/subclusters/18_UMAPCluster_long_IMMUNE_subcluster.pdf",width = 22,height = 12)
...
lapply(res_ids,function(rs){
  ...
  ggsave(plot=test_long01,paste0("../../out/plot/subclusters/18_DotplotLong_",rs,"_IMMUNE_subcluster.pdf"),
         width = 30,
  ...
  pdf(paste0("../../out/plot/subclusters/18_heatmap_signatureLong_",rs,"_IMMUNEsubset.pdf"),
  ...
  pdf(paste0("../../out/plot/subclusters/18_heatmap_signatureLong_",rs,"_IMMUNEsubset2.pdf"),
      width = 6,
  ...
  sobj_total_h.markers %>%
    write_tsv(paste0("../../out/table/subclusters/18_FindAllMarkers_HarmonySample_",rs,"_IMMUNE_subcluster.tsv"))
  
  # pick the top 100 markers per cluster
  sobj_total_h.markers %>%
    group_by(cluster) %>%
    dplyr::slice(1:100) %>%
    write_tsv(paste0("../../out/table/subclusters/18_FindAllMarkers_HarmonySample_",rs,"_IMMUNE_subcluster_top100.tsv"))
  ...
  sobj_total_h.markers %>%
    ...
    write_tsv(paste0("../../out/table/subclusters/18_FindAllMarkers_HarmonySample_",rs,"_IMMUNE_subcluster_top100_noRIBOandMT.tsv"))
  ...
  ggsave(paste0("../../out/plot/subclusters/18_Ditto_HarmonySample_",rs,"_IMMUNEsubset.pdf"),
  ...
  write_tsv(df_summary,paste0("../../out/table/subclusters/18_summary_prop_",rs,"_IMMUNE_subcluster.tsv"))
  ...
  df_summary %>%
    ...
    scale_y_sqrt()
  ggsave(paste0("../../out/plot/subclusters/18_plot_clusterProp_",rs,"_IMMUNEsubset1.pdf"),
