# AIM ---------------------------------------------------------------------
# run the subcluster analysis for the OTHER cells from the WM+CX dataset
...
sobj_subset <- subset(sobj,subset = cell_id %in% c("STROMAL","SCHWANN","EPENDYMA"))
DimPlot(sobj_subset,group.by = "cell_id",label=T,raster = T)

# define the minimal metadata
meta_minimal <- sobj_subset@meta.data %>% select(c(dataset = "project_id","original_sample_name","sample_number","sample_id","nbb","autopsy","cohort","sex","age","braak","amyloid","braaklb","pmd","ph","weight","csf","apoe","barcode","id","tissuecode","iduit","datumuit","uitvraag","recipient","dcodeprot","dcode","diagnosis","dcodewk","wcode","region","location","pathological_stage","demyelination","demyelination.score","specific","ocode","storage","annotation","annotation_confident", "cell_id"))
...
DimPlot(sobj_total_h,raster = T,label = T)
DimPlot(sobj_total_h,raster = T,label = T,group.by = "cell_id")
DimPlot(sobj_total_h,raster = T,group.by = "cell_id",label = T,split.by = "location")

# sobj_total_h <- readRDS("../../out/object/data.combined_harmonySkipIntegration_AllSoupX_01000_6000_15.rds")
saveRDS(sobj_total_h,"../../out/object/17_OTHER_subcluster_HarmonySample.rds")
