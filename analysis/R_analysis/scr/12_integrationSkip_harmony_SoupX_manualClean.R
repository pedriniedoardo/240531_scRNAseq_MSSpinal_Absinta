...
# after the first draft analysis we decided to remove the cell from cluster 9 as they were difficult to anntate. in particular we thimk they are mixed cell populations.
...
data.combined.all <- merge(data.list[[1]], y = data.list[-1], add.cell.ids = data.list.id, project = "spinal")
...
meta_old <- read_tsv("../../out/table/meta_data.combined_harmonySkipIntegration_AllSoupX_00500_07000_05_AnnotationSCType.tsv") %>%
  dplyr::select(rowname,orig.ident,cluster_whitelist = RNA_snn_res.0.1)

# confirm the cluster identity
table(meta_old$cluster_whitelist)

# filter out the cells belonging to the cluster 9. we think they are mixed cell populations based on lineage marker expression
# build the whitelist of the barcodes. in particular filter out all the barcodes associated with the draft analysis
...
meta_new_total <- meta_new %>%
  left_join(meta_old,by = c("rowname","orig.ident"),suffix=c(".new",".old")) %>%
  column_to_rownames("rowname") %>%
  # build the whitelist condition
  mutate(whitelist = case_when(cluster_whitelist %in% c(9)~0,
                               T~1))
...
table(sobj_total@meta.data$cluster_whitelist)
...
saveRDS(sobj_total_h,"../../out/object/data.combined_harmonySkipIntegration_AllSoupX_00500_07000_05_manualClean.rds")
