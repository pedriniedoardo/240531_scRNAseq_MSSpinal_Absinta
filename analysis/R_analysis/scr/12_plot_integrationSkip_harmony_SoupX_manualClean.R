...
# read in the data --------------------------------------------------------
data.combined <- readRDS("../../out/object/data.combined_harmonySkipIntegration_AllSoupX_00500_07000_05_manualClean.rds")
...
# wrangling ---------------------------------------------------------------
# add the missing metadata from Martina. this is the final updated versino generated from Martina
LUT_sample_update <- read_csv("../../data/LUT_sample_full.csv") %>%
  data.frame()

table(LUT_sample_update$demyelination)
table(LUT_sample_update$demyelination.score)
table(LUT_sample_update$pathological_stage)

table(LUT_sample_update$demyelination.score,LUT_sample_update$pathological_stage)
table(LUT_sample_update$demyelination,LUT_sample_update$demyelination.score)
...
clustree::clustree(data.combined@meta.data[,grep("RNA_snn_res", colnames(data.combined@meta.data))],
                   prefix = "RNA_snn_res.")
ggsave("../../out/plot/manualClean/UMAPCluster_tree.pdf",width = 10,height = 10)
...
test_long04 <- DotPlot(data.combined,
  ...
  theme(strip.text = element_text(angle = 90))
ggsave(plot=test_long04,"../../out/plot/manualClean/DotplotLong_harmonySkipIntegration_AllSoupX_00500_07000_05_res0.2.pdf",width = 30,height = 6)
...
ggsave("../../out/plot/manualClean/UMAPCluster_technical.pdf",width = 10,height = 8)
