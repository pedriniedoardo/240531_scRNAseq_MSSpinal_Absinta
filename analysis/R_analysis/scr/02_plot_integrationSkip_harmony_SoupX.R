...
# read in the data --------------------------------------------------------
data.combined <- readRDS("../../out/object/data.combined_harmonySkipIntegration_AllSoupX_00500_07000_05.rds")
Idents(data.combined)<-"seurat_clusters"

# wrangling ---------------------------------------------------------------
# add the missing metadata from Martina
LUT_sample_update <- read_csv("../../data/LUT_sample.csv") %>%
  data.frame()

# add the new meta to the original one
# orig.meta <- data.list$Sample_1@meta.data %>%
#   rownames_to_column("barocode")
orig.meta <- data.combined@meta.data %>%
  rownames_to_column("rownames")

dim(orig.meta)

# swap the original metadata with the new one
data.combined@meta.data <- left_join(orig.meta,LUT_sample_update,by = c("orig.ident" = "sample")) %>%
...
clustree::clustree(data.combined@meta.data[,grep("RNA_snn_res", colnames(data.combined@meta.data))],
                   prefix = "RNA_snn_res.")
ggsave("../../out/plot/UMAPCluster_tree.pdf",width = 10,height = 10)
...
ggsave("../../out/plot/UMAPCluster_resolutions.pdf",width = 20,height = 15)
...
ggsave(plot=test_long01,"../../out/plot/DotplotLong_harmonySkipIntegration_AllSoupX_00500_07000_05_res0.1.pdf",width = 25,height = 5)
...
ggsave(plot=test_long04,"../../out/plot/DotplotLong_harmonySkipIntegration_AllSoupX_00500_07000_05_res0.4.pdf",width = 25,height = 5)
...
ggsave("../../out/plot/UMAPCluster_technical.pdf",width = 10,height = 8)
...
ggsave(plot=plot03,"../../out/plot/UMAPCluster_harmonySkipIntegration_AllSoupX_00500_07000_05.pdf",width = 6,height = 5)
...
ggsave(plot=plot03a,"../../out/plot/UMAPClusterSplit_harmonySkipIntegration_AllSoupX_00500_07000_05.pdf",width = 13,height = 12)

# main umap cell cycle
plot04 <- DimPlot(data.combined, reduction = "umap", group.by = "Phase",raster = T,order = T)
ggsave(plot=plot04,"../../out/plot/UMAPPhase_harmonySkipIntegration_AllSoupX_00500_07000_05.pdf",width = 5,height = 4)
...
ggsave("../../out/plot/BarplotPhase_summary_harmonySkipIntegration_AllSoupX_00500_07000_05.pdf",width = 7,height = 6)
...
df_summary <- df_meta %>%
  ...
  ungroup() %>%
  mutate(prop=n/tot)
write_tsv(df_summary,"../../out/table/summary_harmonySkipIntegration_AllSoupX_00500_07000_05_res01.tsv")
...
ggsave("../../out/plot/Barplot_summary_harmonySkipIntegration_AllSoupX_00500_07000_05_res01.pdf",width = 7,height = 6)
...
df_summary_02 <- df_meta %>%
  ...
  ungroup() %>%
  mutate(prop=n/tot)
write_tsv(df_summary_02,"../../out/table/summary02_harmonySkipIntegration_AllSoupX_00500_07000_05_res01.tsv")
...
df_summary_02 %>%
  ...
  theme(axis.text.x = element_text(hjust = 1,angle = 90))+
  scale_y_sqrt()
ggsave("../../out/plot/BoxplotLocation_summary_harmonySkipIntegration_AllSoupX_00500_07000_05_res01.pdf",width = 8,height = 5)
...
df_summary_02 %>%
  ...
  theme(axis.text.x = element_text(hjust = 1,angle = 90))+
  scale_y_sqrt()
ggsave("../../out/plot/BoxplotDiagnosis_summary_harmonySkipIntegration_AllSoupX_00500_07000_05_res01.pdf",width = 8,height = 5)
...
pdf("../../out/plot/HeatmapCluster_summary_harmonySkipIntegration_AllSoupX_00500_07000_05_res01.pdf",width = 10,height = 6)
...
pdf("../../out/plot/HeatmapSample_summary_harmonySkipIntegration_AllSoupX_00500_07000_05_res01.pdf",width = 10,height = 6)
...
pdf("../../out/plot/HeatmapSampleProp_summary_harmonySkipIntegration_AllSoupX_00500_07000_05_res01.pdf",width = 10,height = 6)
...
sobj_total_h.markers %>%
  write_tsv("../../out/table/FindAllMarkers_harmonySkipIntegration_AllSoupX_00500_07000_05.tsv")
...
ggsave("../../out/plot/TopMarkersDitto_harmonySkipIntegration_AllSoupX_00500_07000_05.pdf",width = 15,height = 6)

# save the object ---------------------------------------------------------
# save the object with the full annotation
saveRDS(data.combined,"../../out/object/data.combined_harmonySkipIntegration_AllSoupX_00500_07000_05_Annotation.rds")
