...
# read in the data --------------------------------------------------------
data.combined <- readRDS("../../out/object/analysis_R44/27_NEU_subcluster_HarmonySample.rds")
# data.combined2 <- readRDS("../../out/object/100_NEU_subcluster_HarmonyRun.rds")
...
clustree::clustree(data.combined@meta.data[,grep("RNA_snn_res", colnames(data.combined@meta.data))],
                   prefix = "RNA_snn_res.")
ggsave("../../out/plot/analysis_R44/27_UMAPCluster_tree_NEU_subcluster.pdf",width = 10,height = 10)
...
ggsave("../../out/plot/analysis_R44/27_UMAPCluster_resolutions_NEU_subcluster.pdf",width = 25,height = 15)
...
ggsave("../../out/plot/analysis_R44/27_UMAPCluster_short_NEU_subcluster.pdf",width = 22,height = 12)
...
ggsave("../../out/plot/analysis_R44/27_UMAPCluster_long_NEU_subcluster.pdf",width = 22,height = 12)
...
ggsave("../../out/plot/analysis_R44/27_UMAPCluster_res0.1_NEU_subcluster.pdf",width = 4,height = 3)

DimPlot(data.combined, reduction = "umap",group.by = "RNA_snn_res.0.1",split.by = "diagnosis_short",label = T,raster = F,ncol=4)
ggsave("../../out/plot/analysis_R44/27_UMAPCluster_splitDisease_res0.1_NEU_subcluster.pdf",width = 8,height = 4)
...
ggsave(plot=test_long01,"../../out/plot/analysis_R44/27_DotplotLong_res0.1_NEU_subcluster.pdf",width = 30,height = 6)
...
sobj_total_h.markers %>%
  write_tsv("../../out/table/analysis_R44/27_FindAllMarkers_HarmonySample_res0.1_NEU_subcluster.tsv")

# pick the top 100 markers per cluster
sobj_total_h.markers %>%
  group_by(cluster) %>%
  dplyr::slice(1:100) %>%
  write_tsv("../../out/table/analysis_R44/27_FindAllMarkers_HarmonySample_res0.1_NEU_subcluster_top100.tsv")
...
sobj_total_h.markers %>%
  ...
  write_tsv("../../out/table/analysis_R44/27_FindAllMarkers_HarmonySample_res0.1_NEU_subcluster_top100_noRIBOandMT.tsv")
...
ggsave("../../out/plot/analysis_R44/27_Ditto_HarmonySample_res0.1_NEU_subcluster.pdf",width = 10,height = 5)
...
#   scale_y_sqrt()
# ggsave("../../out/plot/analysis_R44/27_plot_clusterProp_res0.1_NEU_subcluster.pdf",height = 8,width = 12)
...
out_diagnosis %>%
  ...
  write_tsv("../../out/table/analysis_R44/27_propeller_res0.1_NEU_subcluster.tsv")
...
ggsave("../../out/plot/analysis_R44/27_plot_propeller_res0.1_NEU_subcluster.pdf",height = 5,width = 5)
...
df_summary_diagnosis %>%
  write_tsv("../../out/table/analysis_R44/27_df_summary_diagnosis_res0.1_NEU_subcluster.tsv")
...
df_summary_diagnosis %>%
  ...
  theme_bw()+
  theme(strip.background = element_blank(),axis.text.x = element_text(hjust = 1,angle = 45))
ggsave("../../out/plot/analysis_R44/27_propeller_plot01_res0.1_NEU_subcluster.pdf",width = 9,height = 6)
...
ggsave("../../out/plot/analysis_R44/27_UMAPCluster_res0.4_NEU_subcluster.pdf",width = 6,height = 5)

DimPlot(data.combined, reduction = "umap",group.by = "RNA_snn_res.0.4",split.by = "diagnosis_short",label = T,raster = F,ncol=4)
ggsave("../../out/plot/analysis_R44/27_UMAPCluster_splitDisease_res0.4_NEU_subcluster.pdf",width = 11,height = 5)
# DimPlot(data.combined, reduction = "umap", split.by = "origin",label = T,raster = T,ncol=5)
...
# ggsave(plot=test_long_test,"../../out/plot/analysis_R44/27_DotplotAletta_res0.4_NEU_subcluster.pdf",width = 10,height = 6)
...
test_long01 <- DotPlot(data.combined,
  ...
  labs(title = "RNA_snn_res.0.4")+
  theme(strip.text = element_text(angle = 90))
ggsave(plot=test_long01,"../../out/plot/analysis_R44/27_DotplotLong_res0.4_NEU_subcluster.pdf",width = 30,height = 6)
...
# save the table of all markers
sobj_total_h.markers %>%
  write_tsv("../../out/table/analysis_R44/27_FindAllMarkers_HarmonySample_res0.4_NEU_subcluster.tsv")

# pick the top 100 markers per cluster
sobj_total_h.markers %>%
  group_by(cluster) %>%
  dplyr::slice(1:100) %>%
  write_tsv("../../out/table/analysis_R44/27_FindAllMarkers_HarmonySample_res0.4_NEU_subcluster_top100.tsv")
...
sobj_total_h.markers %>%
  ...
  filter(str_detect(gene,pattern = "^RP[SL][[:digit:]]|^RPLP[[:digit:]]|^RPSA",negate=T)) %>%
  filter(str_detect(gene,pattern = "^HB[^(P)]",negate=T)) %>%
  write_tsv("../../out/table/analysis_R44/27_FindAllMarkers_HarmonySample_res0.4_NEU_subcluster_top100_noRIBOandMT.tsv")
...
ggsave("../../out/plot/analysis_R44/27_Ditto_HarmonySample_res0.4_NEU_subcluster.pdf",width = 15,height = 6)
...
out_diagnosis %>%
  ...
  write_tsv("../../out/table/analysis_R44/27_propeller_res0.4_NEU_subcluster.tsv")
...
speckle::plotCellTypeProps(x = data.combined,
                           ...
                           sample = data.combined$diagnosis_short)+theme_minimal()+theme(panel.grid = element_blank(),axis.text.x = element_text(hjust = 1,angle = 45))
ggsave("../../out/plot/analysis_R44/27_plot_propeller_res0.4_NEU_subcluster.pdf",height = 5,width = 5)
...
df_summary_diagnosis %>%
  write_tsv("../../out/table/analysis_R44/27_df_summary_diagnosis_res0.4_NEU_subcluster.tsv")
...
df_summary_diagnosis %>%
  ...
  theme_bw()+
  theme(strip.background = element_blank(),axis.text.x = element_text(hjust = 1,angle = 45))
ggsave("../../out/plot/analysis_R44/27_propeller_plot01_res0.4_NEU_subcluster.pdf",width = 9,height = 6)
