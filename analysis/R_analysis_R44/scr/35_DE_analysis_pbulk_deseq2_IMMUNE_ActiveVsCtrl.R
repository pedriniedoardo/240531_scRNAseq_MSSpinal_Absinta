...
# the aim in this case is to run the camparison for a subset of samples from the control and the active only
...
meta_summary <- scobj_subset@meta.data %>%
  group_by(sample_id,age,weight,csf,apoe,diagnosis_short,demyelination_short) %>%
  summarise(avg_percent.mt = mean(percent.mt), avg_percent.ribo = mean(percent.ribo))

# wrangling ---------------------------------------------------------------
# explore the full meta
scobj_subset@meta.data %>%
  group_by(sample_id,pathological_stage_short) %>%
  summarise(n = n()) %>%
  pivot_wider(names_from = pathological_stage_short,values_from = n)

# aggregate the expression per sample per donor.id and stimulation
cts_sample_all <- AggregateExpression(object = scobj_subset,
                                      group.by = c("sample_id","sex","pathological_stage_short","location"),
                                      assays = 'RNA',
                                      slot = "counts",
                                      return.seurat = FALSE)
...
meta_reads <- colSums(counts) %>% data.frame(tot_reads = .) %>% mutate(M_reads = tot_reads/10^6) %>% rownames_to_column("sample") %>%
  separate(sample,into = c("sample_id","sex","pathological_stage_short","location"),sep = "_") %>%
  dplyr::select(-c(tot_reads,sex,pathological_stage_short,location)) %>%
  mutate(sample_id = str_replace_all(sample_id,pattern = "-","_"))

# 2. generate sample level metadata
LUT_sample <- scobj_subset@meta.data %>%
  group_by(sample_id,sex,pathological_stage_short,location) %>%
  summarise() %>%
  ...
  # mutate(sample_disease = paste(sample_id_fix,pathological_stage_short,sep = "_")) %>%
  mutate(sample_disease_origin = paste(sample_id_fix,sex,pathological_stage_short,location,sep = "_")) %>%
  ungroup() %>%
...
saveRDS(counts,file = paste0("../../out/object/analysis_R44/35_counts_mono_pBulk_","IMMUNE",".rds"))
saveRDS(colData,file = paste0("../../out/object/analysis_R44/35_colData_mono_pBulk_","IMMUNE",".rds"))

# perform DESeq2 ----------------------------------------------------------
# build the model
pathological <- colData$pathological_stage_short
# gender <- colData$sex
location <- colData$location

# design <- model.matrix(~ block_donor + treat)
# design <- model.matrix(~ gender + location + treat)
design <- model.matrix(~ location + pathological)
colnames(design)[1] <- c("intercept")

# save the disign
saveRDS(design,paste0("../../out/object/analysis_R44/35_design_mono_pBulk","IMMUNE",".rds"))
...
# ggsave(paste0("../../out/plot/analysis_R44/35_pBulk_totReads_","IMMUNE",".pdf"),width = 10,height = 6)
...
# pdf(paste0("../../out/plot/analysis_R44/35_heatmap_SampleCluster_filterExp_","IMMUNE",".pdf"),width = 12,height = 6)
set.seed(21)
...
plot_vsd$data %>%
  separate(name,into = c("sample","sex","pathological_stage","location"),sep = "_",remove = F) %>%
  ggplot(aes(x=PC1,y=PC2,col=demyelination_short)) +
  geom_point(size =3,alpha=0.6) +
  # ggrepel::geom_text_repel(show.legend = F)+
  scale_x_continuous(expand = expansion(mult = 0.1))+
  theme_bw() + ylab(plot_vsd$labels[1]) + xlab(plot_vsd$labels[2])
# ggsave(paste0("../../out/plot/analysis_R44/35_PCA_pseudobulk_filterExp_","IMMUNE",".pdf"),width = 12,height = 10)

plot_vsd$data %>%
  separate(name,into = c("sample","sex","pathological_stage","location"),sep = "_",remove = F) %>%
  mutate(pathological_stage = factor(pathological_stage,levels = c("CTRL","IN","ACT"))) %>%
  ggplot(aes(x=PC1,y=PC2,col=pathological_stage,label = sample)) +
  geom_point(size =3,alpha=0.6) +
  ggrepel::geom_text_repel(show.legend = F)+
  scale_x_continuous(expand = expansion(mult = 0.1))+
  theme_bw() + ylab(plot_vsd$labels[1]) + xlab(plot_vsd$labels[2])
ggsave(paste0("../../out/plot/analysis_R44/35_PCA_pseudobulk_filterExp2_","IMMUNE",".pdf"),width = 12,height = 10)
...
# ggsave(paste0("../../out/plot/analysis_R44/35_PCA_pseudobulk_panel_","IMMUNE",".pdf"),width = 20,height = 20)
...
left_join(test_df1,test_df2,by=c("name")) |> 
  ...
  theme_bw()+theme(strip.background = element_blank(),axis.text.x = element_text(hjust = 1,angle = 45))
# ggsave(paste0("../../out/plot/analysis_R44/35_panel_metadata_PC_pbulk_cat_","IMMUNE",".pdf"),width = 30,height = 15)
...
left_join(test_df3,test_df2,by=c("name")) |> 
  ...
  geom_point(position = position_jitter(width = 0.2))+
  theme_bw()+theme(strip.background = element_blank(),axis.text.x = element_text(hjust = 1,angle = 45))
# ggsave(paste0("../../out/plot/analysis_R44/35_panel_metadata_PC_pbulk_cont_","IMMUNE",".pdf"),width = 30,height = 15)
...
sample_prop_wide <- data.combined@meta.data %>%
  group_by(sample_id,sex,location,pathological_stage_short,cell_id) %>%
  summarise(n = n(),.groups = "drop") %>%
  # match the sample_id with the one in the LUT
  mutate(sample_id_fix = str_replace_all(sample_id,pattern = "_",replacement = "-")) %>%
  mutate(sample_disease_location = paste(sample_id_fix,sex,pathological_stage_short,location,sep = "_")) %>%
...
column_meta_sample_prop <- HeatmapAnnotation(pathological_stage = meta_sample_prop$pathological_stage_short,
                                             location = meta_sample_prop$location,
                                             col = list(pathological_stage = c("CTRL" = "blue",
                                                                               "IN" = "yellow",
                                                                               "ACT" = "red"),
...
# pdf(paste0("../../out/plot/analysis_R44/35_heatmap_cellType_pseudobulk_","IMMUNE",".pdf"),width = 10,height = 8)
...
# pdf(paste0("../../out/plot/analysis_R44/35_heatmap_cellType_pseudobulk2_","IMMUNE",".pdf"),width = 10,height = 8)
...
saveRDS(ddsHTSeq_filter,paste0("../../out/object/analysis_R44/35_ddsHTSeq_pseudobulk_filterExp_","IMMUNE",".rds"))
...
contrast <- makeContrasts(ACT_vs_CTRL = pathologicalACT,
                          IN_vs_CTRL = pathologicalIN,
                          ACT_vs_IN = pathologicalACT - pathologicalIN,
                          levels = design)

# pull the results table
res_ACT <- results(ddsHTSeq_filter, contrast=contrast[,"ACT_vs_CTRL"],alpha = 0.05)
res_IN <- results(ddsHTSeq_filter, contrast=contrast[,"IN_vs_CTRL"],alpha = 0.05)
res_ACTvsIN <- results(ddsHTSeq_filter, contrast=contrast[,"ACT_vs_IN"],alpha = 0.05)
...
df_res <-
  list(res_ACT = res_ACT,
       res_IN = res_IN,
       res_ACTvsIN = res_ACTvsIN) %>%
  lapply(function(x){
    df <- x %>%
      data.frame() %>%
...
df_res %>%
  ...
  write_tsv(paste0("../../out/table/analysis_R44/35_DE_pseudobulk_filterExp_","IMMUNE",".tsv"))
...
# shrink ------------------------------------------------------------------
res_ACT_shr <- lfcShrink(ddsHTSeq_filter, res = res_ACT, type = "ashr")
res_IN_shr <- lfcShrink(ddsHTSeq_filter, res = res_IN, type = "ashr")
res_ACTvsIN_shr <- lfcShrink(ddsHTSeq_filter, res = res_ACTvsIN, type = "ashr")

# save the table of DEGs
df_res_shr <-
  list(res_ACT_shr = res_ACT_shr,
       res_IN_shr = res_IN_shr,
       res_ACTvsIN_shr = res_ACTvsIN_shr) %>%
  lapply(function(x){
    df <- x %>%
      data.frame() %>%
      rownames_to_column(var = "symbol") %>%
...
df_res_shr %>%
  mutate(cluster = "IMMUNE") %>%
  write_tsv(paste0("../../out/table/analysis_R44/35_DE_pseudobulk_filterExp_shr_","IMMUNE",".tsv"))
...
ggsave(paste0("../../out/plot/analysis_R44/35_histogram_pvalue_pseudobulk_filterExp_","IMMUNE",".pdf"),width = 12,height = 4)
...
ggsave(paste0("../../out/plot/analysis_R44/35_vulcano_plot_pseudobulk_filterExp_","IMMUNE",".pdf"),width = 30,height = 10)
...
plot_volcano_shr %>%
  ...
  theme(strip.background = element_blank())+
  theme(legend.position = "none")
ggsave(paste0("../../out/plot/analysis_R44/35_vulcano_plot_pseudobulk_filterExp_shr_","IMMUNE",".pdf"),width = 30,height = 10)
...
column_ha_shr <- HeatmapAnnotation(pathological_stage = meta_sample$pathological_stage_short,
...
pdf(paste0("../../out/plot/analysis_R44/35_heatmap_DEG_plot_pseudobulk_filterExp_shr_","IMMUNE",".pdf"),width = 10,height = 8)
draw(ht2_shr,heatmap_legend_side = "left",annotation_legend_side = "left",padding = unit(c(30,2,2, 2), "mm"))
dev.off()
...
# PLOT DISPERSION ---------------------------------------------------------
pdf(paste0("../../out/plot/analysis_R44/35_dispersion_plot_pseudobulk_filterExp_","IMMUNE",".pdf"),width = 5,height = 5)
