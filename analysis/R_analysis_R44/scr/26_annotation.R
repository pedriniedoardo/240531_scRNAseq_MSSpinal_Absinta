...
# read in the data --------------------------------------------------------
scobj <- readRDS("../../out/object/analysis_R44/24_sobj_integrated_cleanup.rds")
...
scobj$diagnosis_short <- scobj@meta.data$diagnosis %>% factor(levels = c("Non-demented control","Multiple sclerosis"),labels = c("CTRL","MS"))
scobj$pathological_stage_short <- scobj@meta.data$pathological_stage %>% factor(levels = c("control","inactive","active demyelination"),labels = c("CTRL","IN","ACT"))
scobj$demyelination_short <- scobj@meta.data$demyelination %>% factor(levels = c("no demyelination","less than half","more than half"),labels = c("NO","LESS50","MORE50"))
...
# save the object
saveRDS(scobj,"../../out/object/analysis_R44/26_sobj_integrated_cleanup_manualAnnotation.rds")
